...
| Zustand | Verhalten |
| --- | --- |
| **Variable gesetzt** | Beim App-Start erscheint ein Lock-Screen. Erst nach dem Passwort werden die Factory (**UI und alle `/api`-Routen**: tts, gate) freigeschaltet. Die signierte HttpOnly-Session-Cookie hält standardmäßig **7 Tage** — kein Neu-Fragen bei jedem Reload. Passwort ändern in Vercel → alle Sessions sofort ungültig. |
...
Session schaltet dann auch `/api/tts` frei
...
│  ├─ GatePanel.tsx            lock screen + gate banner (APP_PASSWORD)
...
└─ lib/
   ...
   ├─ gate.ts       access-gate client (cookie session via /api/gate)
   ├─ cfr.ts        ffmpeg.wasm CFR re-encode with timeouts + fallback
   └─ storage.ts    IndexedDB asset vault (background clips + music)

api/        ← tts relay + gate (Vercel Serverless, Node.js runtime)
