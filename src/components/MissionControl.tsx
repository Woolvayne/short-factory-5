...
import type { PostMode } from "../lib/posts";
...
export function OutputPanel({
  ...
  /** Versand läuft ausschließlich über Buffer */
  ...
  const post = (targets: LocalRenderItem[]) => {
                    ...
                    {item.status === "done" &&
                      (item.posting ? (
                        ...
                        <div className="flex min-h-[34px] w-full items-center justify-center gap-1.5 border border-volt-400/60 bg-volt-400/10 px-2 py-1.5 font-mono text-[9.5px] font-bold tracking-widest text-volt-300">
                          <Check className="size-3" strokeWidth={2.5} /> GEPOSTET · BUFFER
                        </div>
                      ) : (
                        <button
                          type="button"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {doneCount > 0 && (
              <button
                type="button"
                onClick={() => post(items.filter((i) => i.status === "done"))}
                disabled={postingCount > 0}
                title="Alle fertigen Videos über Buffer posten"
